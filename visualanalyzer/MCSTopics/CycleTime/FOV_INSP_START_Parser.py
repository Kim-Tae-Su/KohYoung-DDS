...
from MCSTopics.CycleTime.DDS_FOV_INSP_START import * 
...
class CFovInspStartParser(CDataExportThread, CJasonManager):
    ...
    def initDdsHandler(self):
        ...
        msg = ""
        try:
            self._ddsHandler = DdsFovInspStartHandler(self)
    ...
    def updateDdsFovMovingTImeData(self, domain, topic, cmd, year, month, day, hour, minute, seconds, millisconds, seqId, userIndex, coordinate):
        result = True
        msg = ""
        try:
            data = CFovMovingTimeData(
                domain,
                topic,
                cmd,
                datetime.strptime("{}/{}/{}_{}:{}:{}:{}".format(year, month, day, hour, minute, seconds, millisconds), DATETIME_FORMAT),
                seqId, userIndex, coordinate)
    ...
    def export_data(self, ct_data: Union[CCycleTimeData, CFovMovingTimeData]):
        result = True
        msg = ""
        try:
            data = list()
            if isinstance(ct_data, CFovMovingTimeData):
                data.append({
                ...
                'seq' : ct_data.seq,
                'state' : ct_data.state,
                'coordinate' : ct_data.coordinate
                })
            elif isinstance(ct_data, CCycleTimeData):
                data.append({
                'domain' : ct_data.domain,
                'topic' : ct_data.topic,
                'cmd' : ct_data.cmd,
