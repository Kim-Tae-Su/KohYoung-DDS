...
from KYITopics.DdsGrabStart import * 
...
class CGrabStartParser(CDataExportThread, CJasonManager):
    ...
    def initDdsHandler(self):
        ...
        msg = ""
        try:
            self._ddsHandler = DdsGrabStartHandler(self)
    ...
    def updateDdsData(self, domain, topic, cmd, year, month, day, hour, minute, seconds, millisconds, \
        ...
        try:
            data = CGrabStartData(
                ...
                int(seqId), int(userIndex), bool(isWait), grabMode, userMemory, int(float(userMemorySize)))
