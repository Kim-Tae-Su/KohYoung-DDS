...
class DdsLaneInspStartHandler(McsTopic):
        ...
        try:
            if self.recvCommand == "MCStoGUI":
                if isinstance(self.recvParam, bytes):
                    ...
                    seqId = "LANE_INSP"+params[0]
                    DdsLaneInspStartHandler.isDualLane = True
                    DdsLaneInspStartHandler.InspLane = params[0]
                else:
                    result = False
                    msg = "Error: 'params' list is empty."
                userIndex = 0
